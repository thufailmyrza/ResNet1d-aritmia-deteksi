...
def freeze_backbone(model, unfreeze_last_stage: bool = False):
    """
    Freeze all layers except head classifier.
    Opsional: unfreeze stage5 (depeest ResNet layer).
    Strategi:
      head only        
      head + stage5   
      all layer       → full fine-tuning
    """
    # Freeze all
    ...
    # Opsional: unfreeze stage5 backbone
    if unfreeze_last_stage:
        ...
        print("  ✓ Freeze: trained only head classifier")
...
def fine_tune(args: argparse.Namespace):
    ...
    if not entries:
        print("\n  NO FILES FOUND. CHECH --data-dir AND --mode.")
        return
    print(f"\n  TOTAL FILES FOUND: {len(entries)}")
    ...
    if can_stratify and len(entries) >= 4:
        ...
        print("  DATA TOO SMALL FOR STRATIFIED SPLIT. PERFORMING RANDOM SPLIT.")
        train_entries, val_entries = train_test_split(
    ...
    if len(train_ds) == 0:
        print("  NO TRAINING WINDOWS FOUND. CHECK FILE .bin AND ADC FORMAT.")
        return
    ...
    print(f"\n  ── Sanity Check ──")
    print(f"  Loss          : {sanity_loss.item():.4f}  (expected ≈ {expected:.4f})")
    print(f"  Label range   : {sample_lbl.min().item()} – {sample_lbl.max().item()}")
    if sanity_loss.item() > 8.0:
        print(" HIGH LOSS — CHECK ADC FORMAT (--adc-format)")
    elif sanity_loss.item() < 0.3:
        print(" LOW LOSS — OVERFITTING")
    else:
        print(" LOSS NORMAL")
    model.train()
    class_weights = torch.ones(NUM_ARRHYTHMIA_CLASSES, dtype=torch.float32)
    class_weights[0]  = args.w_normal         # normal
    class_weights[8]  = args.w_tachy          # tachycardia
    class_weights[10] = args.w_af             # atrial fibrillation 
    class_weights[4]  = args.w_quadrigeminy   # quadrigeminy 
    ...
    print(f"  Before  (PTB-XL val) : {prev_best_f1:.4f}")
